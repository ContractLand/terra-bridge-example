...
async function main() {
  ...
  console.log(`Home (Terra) CLC (native) balance: ${web3Home.utils.fromWei(homeEth).toString()} ETH`)

  const homeToken = await homeTokenContract.methods.balanceOf(USER_ADDRESS).call()
  console.log(`Home (Terra) ETH (token) balance: ${web3Home.utils.fromWei(homeToken)}`)
  ...
  const erc20TokenHome = await erc20TokenHomeContract.methods.balanceOf(USER_ADDRESS).call()
  console.log(`Home (Terra) ERC20 token balance: ${web3Home.utils.fromWei(erc20TokenHome)}`)

  const erc20TokenForeign = await erc20TokenForeignContract.methods.balanceOf(USER_ADDRESS).call()
  console.log(`Foreign (Ropsten) ERC20 token balance: ${web3Foreign.utils.fromWei(erc20TokenForeign)}`)
}
main()
